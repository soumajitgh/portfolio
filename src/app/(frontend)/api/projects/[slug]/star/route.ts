...
async function getPublishedProjectID(slug: string) {
...
async function getState(projectID: number, visitorHash: string) {
  const payload = await getPayload({ config })
  const [visitorStars, count] = await Promise.all([
    payload.find({
      ...
      select: { project: true },
      where: {
        and: [{ project: { equals: projectID } }, { visitorHash: { equals: visitorHash } }],
      },
    }),
    payload.count({
      collection: 'project-stars',
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ slug: string }> }) {
  try {
    ...
    const state = await getState(projectID, visitor.hash)
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ slug: string }> }) {
  ...
  try {
    ...
    if (!projectID) return errorResponse('Project not found', 404)
    const visitor = visitorIdentity(request)
    if (!withinRateLimit(`${projectID}:${visitor.hash}`))
    ...
    if (!state.starred) {
      try {
        await payload.create({
          collection: 'project-stars',
          data: { project: projectID, visitorHash: visitor.hash },
          overrideAccess: true,
        })
      } catch {
    ...
    const updated = await getState(projectID, visitor.hash)
...
export async function DELETE(
  ...
  try {
    ...
    const state = await getState(projectID, visitor.hash)
    if (state.starID) {
      await payload.delete({ collection: 'project-stars', id: state.starID, overrideAccess: true })
    }
    const updated = await getState(projectID, visitor.hash)
    revalidateProject(slug)
    return withVisitorCookie(
